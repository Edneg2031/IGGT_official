...
class MemEffAttention(Attention):
    ...
    def forward(self, x, xpos):
        ...
        x = xformers.ops.memory_efficient_attention(q, k, v, attn_bias=None).reshape(B, N, C)
...
class CrossAttention(nn.Module):
    ...
    def __init__(self, dim, rope=None, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., use_xformers=False, qk_norm=False):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.head_dim = head_dim
...
class MemEffCrossAttention(CrossAttention):
    ...
    def forward(self, query, key, value, qpos=None, kpos=None):
        ...
        x = memory_efficient_attention(q, k, v, scale=self.scale).reshape([B, Nq, C]) # memory, 3303
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.attn = MemEffAttention(dim, rope=rope, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, qk_norm=qk_norm)
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.cross_attn = MemEffCrossAttention(dim, rope=rope, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, qk_norm=qk_norm)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
